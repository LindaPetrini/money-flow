...
- [x] **INFRA-01**: App scaffolds with Vite 7 + React 19 + TypeScript, runs via `npm start` on localhost with no server
- [x] **INFRA-02**: App is open-source publishable (no proprietary dependencies, clean GitHub repo)
- [x] **INFRA-03**: File System Access API integration — user grants directory access once per session; handle stored in IndexedDB for re-use
...
- [x] **INFRA-06**: All money arithmetic uses integer cents throughout (`parseCents`, `formatCents`, `splitCents` with largest-remainder) — no floating point in domain logic
- [x] **INFRA-07**: Data persisted as human-readable JSON files in user-selected directory via FSA
...
|-------------|-------|--------|
| INFRA-01 | Phase 1 (implemented) | Complete |
| INFRA-02 | Phase 1 (implemented) | Complete |
| INFRA-03 | Phase 1 (implemented) | Complete |
| INFRA-04 | Phase 1 | Complete |
| INFRA-05 | Phase 1 | Complete |
| INFRA-06 | Phase 1 (implemented) | Complete |
| INFRA-07 | Phase 1 (implemented) | Complete |
