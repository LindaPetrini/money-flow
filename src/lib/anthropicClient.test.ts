 ...
 *
 * Tests callCombinedAnalysis() and callFloorDetection() in isolation
...
import {
  ...
  type FloorItemSuggestion,
  type FloorDetectionResult,
...
const SAMPLE_FLOOR_RESULT: FloorDetectionResult = {
  suggestions: [
    {
      name: 'Netflix',
      amountEur: 15.99,
      frequency: 'monthly',
      confidence: 'high',
      reason: 'Appears monthly with consistent amount',
    },
  ],
};
...
describe('callCombinedAnalysis', () => {
  ...
  it('uses json_schema output_config', async () => {
    const mockFetch = mockFetchSuccess(SAMPLE_COMBINED_RESULT);
    vi.stubGlobal('fetch', mockFetch);
...
describe('callFloorDetection', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  const CLARIFICATIONS = [
    { merchantName: 'NETFLIX SUBSCRIPTION', bucketAccountId: 'fun', context: 'Streaming' },
  ];

  it('returns FloorDetectionResult on success', async () => {
    vi.stubGlobal('fetch', mockFetchSuccess(SAMPLE_FLOOR_RESULT));

    const result = await callFloorDetection('key', SAMPLE_TRANSACTIONS, CLARIFICATIONS);

    expect(result.suggestions).toHaveLength(1);
    expect(result.suggestions[0].name).toBe('Netflix');
    expect(result.suggestions[0].amountEur).toBe(15.99);
    expect(result.suggestions[0].frequency).toBe('monthly');
  ...
  it('calls fetch with correct headers', async () => {
    const mockFetch = mockFetchSuccess(SAMPLE_FLOOR_RESULT);
    vi.stubGlobal('fetch', mockFetch);

    await callFloorDetection('floor-key', SAMPLE_TRANSACTIONS, CLARIFICATIONS);

    expect(mockFetch).toHaveBeenCalledOnce();
    const [url, options] = mockFetch.mock.calls[0];
    expect(url).toBe('https://api.anthropic.com/v1/messages');
    expect(options.headers['x-api-key']).toBe('floor-key');
    expect(options.headers['anthropic-dangerous-direct-browser-access']).toBe('true');
  });
  ...
  it('uses max_tokens: 2048', async () => {
    ...
    await callFloorDetection('key', SAMPLE_TRANSACTIONS, CLARIFICATIONS);

    const body = JSON.parse(mockFetch.mock.calls[0][1].body);
    expect(body.max_tokens).toBe(2048);
  });

  it('works with empty clarifications array', async () => {
    const mockFetch = mockFetchSuccess(SAMPLE_FLOOR_RESULT);
    vi.stubGlobal('fetch', mockFetch);

    const result = await callFloorDetection('key', SAMPLE_TRANSACTIONS, []);
  ...
  it('throws AnthropicAPIError on non-2xx response', async () => {
    vi.stubGlobal('fetch', mockFetchError(500, 'overloaded_error'));

    await expect(callFloorDetection('key', SAMPLE_TRANSACTIONS, [])).rejects.toThrow(
      AnthropicAPIError,
    );
  });

  it('AnthropicAPIError has correct status and errorType', async () => {
    vi.stubGlobal('fetch', mockFetchError(503, 'overloaded_error'));

    try {
      await callFloorDetection('key', SAMPLE_TRANSACTIONS, []);
      expect.fail('Should have thrown');
    } catch (err) {
      expect(err).toBeInstanceOf(AnthropicAPIError);
      expect((err as AnthropicAPIError).status).toBe(503);
      expect((err as AnthropicAPIError).errorType).toBe('overloaded_error');
    }
  });
  ...
  it('does NOT include apiKey in thrown error message', async () => {
    ...
    try {
      await callFloorDetection('super-secret-floor-key', SAMPLE_TRANSACTIONS, []);
      expect.fail('Should have thrown');
    } catch (err) {
      expect((err as Error).message).not.toContain('super-secret-floor-key');
    }
  });

  it('throws generic Error on network failure', async () => {
    vi.stubGlobal('fetch', mockFetchNetworkFailure('fetch failed'));

    await expect(callFloorDetection('key', SAMPLE_TRANSACTIONS, [])).rejects.toThrow('fetch failed');
  });
});
...
describe('COMBINED_ANALYSIS_SCHEMA structure', () => {
  it('exported types are well-shaped (compile-time check via type inference)', () => {
    ...
    const suggestion: FloorItemSuggestion = {
      name: 'Rent',
      amountEur: 1200,
      frequency: 'monthly',
      confidence: 'high',
      reason: 'appears every month',
    };

    const floorResult: FloorDetectionResult = {
      suggestions: [suggestion],
    };

    expect(tx.description).toBe('TEST');
    expect(suggestion.amountEur).toBe(1200);
    expect(floorResult.suggestions).toHaveLength(1);
  });
});
