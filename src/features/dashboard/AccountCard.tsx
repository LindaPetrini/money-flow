...
const PROGRESS_BAR_CLASSES: Record<Status, string> = {
  'at-target': 'bg-green-500',
  'near-target': 'bg-yellow-400',
  'below-target': 'bg-red-500',
};
...
export function AccountCard({ account, onBalanceChange }: AccountCardProps) {
  ...
  const barClass = PROGRESS_BAR_CLASSES[status];
  const progressPct = account.targetCents > 0
    ? Math.min(100, (account.balanceCents / account.targetCents) * 100)
    : 0;
  ...
  return (
    <Card className="py-0">
      <CardContent className="px-4 py-3">
        ...
        {/* Progress bar + target (only when target is set) */}
        {account.targetCents > 0 && (
          <div className="mt-2 pl-4 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {formatCents(account.balanceCents as Cents)} / {formatCents(account.targetCents as Cents)}
              {account.balanceCents >= account.targetCents && ' — at target'}
            </p>
          </div>
        )}
      </CardContent>
