    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-1 pt-2 border-t border-border">
        <div className="flex items-center gap-2">
          ...
          {isOver && (
            <span className="text-xs text-destructive">Exceeds 100% — reduce some values</span>
          )}
        </div>
        {!isOver && remaining > 0.005 && (
          <p className="text-xs text-muted-foreground">
            {remaining.toFixed(2)}% unallocated — stays in your income account after tax
          </p>
        )}
      </div>
